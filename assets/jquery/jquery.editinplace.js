...
$.extend(InlineEditor.prototype, {
	...
	removeHoverEffect: function() {
		if (this.settings.hover_class)
			...
			// var bg = this.settings.bg_out;
			/* linea agregada por bot */
			bg = this.dom.closest('tr').hasClass('even') ? '#f9f9f9' : '#ffffff';
			this.dom.css("background-color", bg);
		}
	},
	...
	handleCancelEditor: function(anEvent) {
		...
		/* lineas cambiadas por bot (bug encontrado) */
		if (hasContent(this.originalValue)
	...
	handleSaveEditor: function(anEvent) {
		...
		/* agregado por bot */
		if (enteredText.length == 0) enteredText = '0';
